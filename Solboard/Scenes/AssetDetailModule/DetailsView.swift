...
struct DetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack() {
                if detailItem.imageUrl.isSolanaURL() {
                    Image(.solana)
                        .resizable()
                        .frame(width: 310, height: 310)
                        .padding(.horizontal, 40)
                } else {
                    KFAnimatedImage(detailItem.imageUrl)
                        .placeholder {
                            ProgressView()
                ...
                if detailItem.balance > 0 || detailItem.pricePerToken > 0 {
                    HStack {
                        Text("Token info")
                            .padding(.leading, 20)
                        ...
                        HStack {
                            ...
                            Text(detailItem.formattedAddress)
                                ...
                                .onTapGesture(perform: {
                                    detailItem.copyAddressToClipboard()
                                })
                        }
...
struct DetailsView: View {
    let detailItem: DetailItemViewModel
...
#Preview {
    ...
    return DetailsView(detailItem: createDummyNFT(json: tokenJson)!)
}
